...
function applyTheme(theme, saveLocal = true) {
    ...
    if (metaTheme) metaTheme.setAttribute("content", THEME_META[value]);

    if (saveLocal) localStorage.setItem(THEME_STORAGE_KEY, value);
    return value;
}
...
function applySettingsToUi() {
    ...
    if (privacySettings) {
        privacySettings.value = s.allow_friend_requests || "everyone";
    }
    ...
    if (dndEnabled) dndEnabled.checked = !!s.dnd_enabled;
    if (dndTimeSettings) {
        dndTimeSettings.style.display = dndEnabled?.checked ? "block" : "none";
    }
    if (dndStartTime) dndStartTime.value = s.dnd_start_time || "22:00";
    if (dndEndTime) dndEndTime.value = s.dnd_end_time || "08:00";

    const themeSelect = qs("themeSelect");
    const activeTheme = applyTheme(
        s.theme || localStorage.getItem(THEME_STORAGE_KEY) || "default",
        true,
    );
...
function installResponsiveEnvironment() {
    ...
    if (!window.__lanMessengerViewportBound) {
        ...
        window.addEventListener("resize", () => syncMembersSheetLayout(true));
...
function setChatHeader(chat) {
    ...
    if (!group) state.ui.membersOpen = false;
    syncMembersSheetLayout(true);
    setEmptyState(!chat);
}
...
function syncMembersToggleButton() {
    const btn = qs("btnShowMembers");
    const group = state.currentChat?.type === "group";
    if (!btn) return;
    btn.classList.toggle("active", !!state.ui.membersOpen && group);
    btn.setAttribute("aria-pressed", String(!!state.ui.membersOpen && group));
    btn.title = !group
        ? "Участники"
        : state.ui.membersOpen
          ? "Скрыть состав"
          : "Показать состав";
}

function clearMembersCloseTransition() {
    const panel = qs("groupMembersPanel");
    if (!panel) return;
    if (panel.__membersCloseHandler) {
        panel.removeEventListener("transitionend", panel.__membersCloseHandler);
        panel.__membersCloseHandler = null;
    }
    if (panel.__membersCloseTimer) {
        clearTimeout(panel.__membersCloseTimer);
        panel.__membersCloseTimer = null;
    }
}

function syncMembersSheetLayout(immediate = false) {
    const panel = qs("groupMembersPanel");
    const backdrop = qs("membersBackdrop");
    const stack = document.querySelector(".chat-stack");
    const group = state.currentChat?.type === "group";
    const open = !!state.ui.membersOpen && group;
    if (!panel || !stack) return;

    if (!group) {
        stack.classList.remove("members-open");
        panel.classList.remove("members-modal-open", "sheet-visible");
        hide(panel);
        if (backdrop) {
            backdrop.classList.remove("open");
            hide(backdrop);
        }
        syncMembersToggleButton();
        return;
    }
    ...
    clearMembersCloseTransition();
    show(panel);
    panel.classList.add("members-modal-open");
    stack.classList.add("members-open");

    if (isDesktopMembersLayout()) {
        if (backdrop) {
            backdrop.classList.remove("open");
            hide(backdrop);
        }
        if (immediate) {
            panel.classList.add("sheet-visible");
        } else {
        ...
        if (backdrop) {
            show(backdrop);
            if (immediate) backdrop.classList.add("open");
...
function openMembersSheet() {
    if (state.currentChat?.type !== "group") return;
    state.ui.membersOpen = true;
    syncMembersSheetLayout(false);
    document.addEventListener("keydown", _membersEscHandler);
}

function closeMembersSheet(options = {}) {
    const { immediate = false } = options;
    const panel = qs("groupMembersPanel");
    const backdrop = qs("membersBackdrop");
    const stack = document.querySelector(".chat-stack");
    state.ui.membersOpen = false;
    if (!panel) return;

    panel.classList.remove("sheet-visible");
    if (backdrop) backdrop.classList.remove("open");
    if (stack) stack.classList.remove("members-open");
    ...
    panel.__membersCloseHandler = finishClose;
    panel.addEventListener("transitionend", finishClose);
    syncMembersToggleButton();
    panel.__membersCloseTimer = setTimeout(() => {
        if (!panel.classList.contains("sheet-visible")) finishClose();
    }, 420);
}

function toggleMembersSheet() {
    if (state.ui.membersOpen) closeMembersSheet();
...
function bindUi() {
    ...
    settingsSave.onclick = async () => {
        try {
            state.settings = await api("/api/settings", {
                method: "POST",
                body: JSON.stringify({
                    allow_friend_requests: qs("privacySettings")?.value || "everyone",
                    ...
                    theme: qs("themeSelect")?.value || "default",
                }),
            });

            // ✅ ВОТ ЭТА СТРОКА — СРАЗУ ПОСЛЕ ОТВЕТА СЕРВЕРА
    ...
    if (btnShowMembers) btnShowMembers.onclick = () => toggleMembersSheet();
