const state = {
  ...
  function messageBody(m) {
    ...
    if (m.kind === "video" || m.kind === "circle") return `${text}<br><video src="${fileUrl}" controls playsinline webkit-playsinline preload="metadata"></video>`;
    if (m.kind === "voice") return `${text}<br><audio src="${fileUrl}" controls preload="metadata"></audio>`;
  ...
  function ensureCallTile(userId, isLocal = false) {
    ...
    vol.oninput = () => { video.volume = Number(vol.value); };
    ...
    volBtn.onclick = () => { if (!isLocal) vol.classList.toggle("hidden"); };
  ...
  // === ИСПРАВЛЕННАЯ ФУНКЦИЯ (ПОДАВЛЕНИЕ КОЛЛИЗИЙ) ===
  async function handleSignal(fromUser, signal) {
    if (!state.call.active) return;
    fromUser = Number(fromUser);
    
    const isPolite = state.me?.id < fromUser;
    const pc = await ensurePeer(fromUser, false); // createOffer = false, так как мы уже в процессе обработки сигнала
    ...
    try {
        if (signal.type === "offer") {
            ...
            try {
                ...
                if (!pc._ignoreOffer) console.error("ICE error:", e);
            }
        }
    } catch (e) {
